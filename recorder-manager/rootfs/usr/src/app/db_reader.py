    ...
    def _query_entity_stats(self) -> dict:
        ...
        try:
            ...
            try:
                ...
                # Calculate lifetime average writes per minute
                if has_states_meta:
                    cursor.execute("""
                        SELECT sm.entity_id,
                               COUNT(s.state_id) AS cnt,
                               MAX(s.last_updated_ts) - MIN(s.last_updated_ts) AS span_s
                        FROM states s
                        JOIN states_meta sm ON s.metadata_id = sm.metadata_id
                        GROUP BY sm.entity_id
                        HAVING COUNT(s.state_id) > 1
                    """)

                    for row in cursor.fetchall():
                        entity_id = row["entity_id"]
                        span_s = row["span_s"] or 0
                        if entity_id in stats and span_s > 0:
                            stats[entity_id]["writes_per_minute"] = (
                                float(row["cnt"]) / (span_s / 60.0)
                            )
            finally:
                conn.close()
